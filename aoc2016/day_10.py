...
###	{"bot-37": {"values": [], "high": "bot-xx", "low": "bot-yy"}, ...}
...
def convert_data(data):
	...
	for l in iter(data.splitlines()):
		...
		if ls[0]=="bot":
			...
			high=ls[10]+"-"+ls[11]
			for t in (low,high):
				if t not in jsonData:
					jsonData[t] = {"values": []}
			if bot in jsonData:
				jsonData[bot]["high"]=high
				jsonData[bot]["low"]=low
			else:
				jsonData[bot] = {"values": [], "high": high, "low": low}
		elif ls[0]=="value":
			val=int(ls[1])
			bot=ls[-2]+"-"+ls[-1]
			if bot in jsonData:
				jsonData[bot]["values"].append(val)
			else:
				jsonData[bot] = {"values": [val]}
	return jsonData

def main():
	jD = convert_data(data)
	bot = next((b for b in jD if len(jD[b]["values"])==2), None)
	while bot:
		if 17 in jD[bot]["values"] and 61 in jD[bot]["values"]:
			print(f"Answer One: the bot with chips {jD[bot]['values']} is {bot}")
		jD[bot]["values"].sort()
		jD[jD[bot]["high"]]["values"].append(jD[bot]["values"].pop())
		jD[jD[bot]["low"]]["values"].append(jD[bot]["values"].pop())
		bot = next((b for b in jD if len(jD[b]["values"])==2), None)
	mul = 1
	for o in ["output-0", "output-1", "output-2"]:
		mul *= jD[o]["values"][0]
	print(f"The multiplication of the values is {mul}")

data="""bot 37 gives low to bot 114 and high to bot 150
