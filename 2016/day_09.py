...
def string_to_entity_array(s):
	...
	while i < l:
		if s[i] == "(":
			_e = s.find(")", i+1)
			_a,_m = map(int, s[i:_e+1].strip("()").split("x"))
			res.append(create_entity(length=_e+1-i, area=_a, mul=_m))
			i = _e+1
		elif s[i] in TEXT_CHARS:
			j=1
			while i+j < l and s[i+j] in TEXT_CHARS:
				j+=1
			res.append(create_entity(length=j))
			i += j
	return res

def create_entity(length, area=0, mul=0):
	return {'l': length, 'a': area, 'm': mul}

# TODO
# something went wrong, decompressing can be change the area-tag of the forward multipler elements,
# but I do not know why...
# CAUSE: there is only a reference copy in the entity list :( (with dc, more then one element changes)
def decompress_step(L, debug=False):
	if L[0]['a'] == 0 and L[1]['a'] == 0:
		sum_i = next((i for i,e in enumerate(L) if e['a']), None)
		sum_length=sum([e['l'] for e in L[:sum_i]])
		del L[:sum_i]
		L.insert(0, create_entity(length=sum_length))
		return True
	if L[0]['a']:
		dc = L.pop(0).copy()
		for i,e in enumerate(L):
			if e['l'] <= dc['a']:
				...
				L.insert(i,create_entity(length=dc['a']))
				L[i+1]['l'] -= dc['a']
				dc['a'] = 0
			if dc['a'] == 0:
				break
		...
		if debug:
			for i,e in enumerate(L):
				if e['m'] > 0 and e['a'] == 0:
					print("ERROR after L[0]['a'] at element {} what is {}".format(i,e))
					_ = input("      press enter...")
		return True
	elif L[1]['a']:
		dc = L.pop(1).copy()
		for i,e in enumerate(L):
			...
			if dc['a'] == 0:
				break
		L[:] = L[:1] + L[1:i+1]*(dc['m']-1) + L[1:]
		if debug:
			for i,e in enumerate(L):
				if e['m'] > 0 and e['a'] == 0:
					print("ERROR after L[1]['a'] at element {} what is {}".format(i,e))
					_ = input("      press enter...")
		return True
	return False

def main():
	print("Officinal answer one:",decompress(data,False))
	print("Officinal answer two:",decompress(data,True))
	test("(3x3)XYZ")
	_ = input("press enter...")
	...
	data_array=[]
	data_array.extend(string_to_entity_array(data))
	#print(data_array[:10])
	i=0
	while len(data_array) > 1 and decompress_step(data_array):
		i+=1
		#print('.',end="")
		if i % 1000 == 0:
			print(f".{len(data_array)}",end="")
			if i % 100000 == 0:
				print(f"\n*** Step {i} ***")
	print("\nTotal uncompressed length: {}".format(data_array[0]['l']))

def test(data):
	entity_array = string_to_entity_array(data)
	i=0
	while len(entity_array) > 1 and decompress_step(entity_array, False):
		print('.',end="")
		i+=1
		if i >= 1000:
			print(f"{len(entity_array)}",end="")
			i=0
		#print("num of entities: {}".format(len(entity_array)))
		#print(entity_array)	
	print("\nTotal uncompressed length of {} is: {}".format(data, entity_array[0]['l']))
